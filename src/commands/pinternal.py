...
def func(root, display, *args, **kwargs):
    ...
    def print_to_console(indent, text, prefix):
        try:
            display.pprint(indent + text + f"{''.join([' ' for ch in range(60) if len(indent + text) + ch < 60])} ({prefix})")
        except TypeError:
            text = str(text)
            display.pprint(indent + text + f"{''.join([' ' for ch in range(60) if len(indent + text) + ch < 60])} (class)")
    ...
    try:
        ...
        display.pprint(f"Unknown Var: {args[0]}")
