...
set_dir_entry = tk.Entry(gui,bd=1,font="monospace 8",width=60)
...
for i ,command in enumerate(LIST_OF_COMMANDS):
    command_name = tk.Label(gui,text=command,font="monospace 10",fg="darkblue").grid(row=ROW_NUM,column=i * 3,sticky="nswe")
    command_button = tk.Button(gui,text=f"Run {command}",font="monospace 9").grid(row=ROW_NUM,column=i * 4 + 1 ,sticky="nswe")
    try:
