...
class Tab(tk.Frame):
    def __init__(self, root, tkroot): 
        ...
        def tkinputconsole_keyevents(event):
            if event.keysym == "Return":
                ...
                root["run_command"](root, 
                                 root["loaded_tabs"][CFG.output_tab], raw_command.split(" ")[0] if isinstance(raw_command.split(" "), list) else raw_command, 
                                 raw_command.split(" ")[1:] if isinstance(raw_command.split(" "), list) else [])
            else:
                pass
        ...
        # Right (User Input)
        self.tkinputconsole_bottom = TKText(self, width=80, height=12, wrap=tk.NONE, font='monospace 10', bg="black", fg="white")
        self.tkinputconsole_bottom.grid(column=0, row=51, rowspan=50)
        self.tkinputconsole_bottom.bind("<Key>", tkinputconsole_keyevents)
        
        # Left
        self.tkinputconsole_top = TKText(self, width=80, height=12, wrap=tk.NONE, font='monospace 10', bg="black", fg="white")
