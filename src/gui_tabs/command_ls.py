...
from pyassets.tk import create_combobox, TKScrollableFrame
...
class Tab(TKScrollableFrame):
    def __init__(self, root, tkroot): 
        TKScrollableFrame.__init__(self, tkroot)
        self.root = root
        
        # Create a Entrybox with CWD inserted
        tk.Label(self.interior, text="set directory: ",font="monospace 8").grid(row=0,column=0)
        self.set_dir_entry = create_combobox(self.interior, [root["cwd"]], font="monospace 8", width=60, state="readonly")
        ...
        for i, command in enumerate(root["imported_commands"].keys()):
            try:
                ...
                pass
            args_entry = create_combobox(self.interior, [""], font="monospace 8")
