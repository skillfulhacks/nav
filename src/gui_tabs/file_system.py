...
import os
from tk_custum_widgets import TKListbox, create_textbox
...
class Tab(tk.Frame):
    def __init__(self, root, tkroot):
                ...
                if index_meta_data["opened"] == False:
                    try:
                        for i, item in enumerate(os.listdir(os.path.join(root["cwd"], index_meta_data['path']))):
                            self.meta_data.insert(i + index + 1, {"path": f"{index_meta_data['path']}{root['slash']}{item}",
        ...
        self.current_dir = create_textbox(self, self.past_dirs, width=40)
        ...
        # File GUI
        self.box = TKListbox(self, width=80, height=18)
